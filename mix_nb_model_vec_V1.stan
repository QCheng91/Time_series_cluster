...
transformed parameters{
  ...
  simplex[c] theta; //simplex[c] theta[d];
  ...
  theta[c]= 1-sum(theta[1:(c-1)]); // to make a simplex.
...
//  vector[N] log_lik_clust_mat[c];
 ...
 for(tp in 1:d) {
    ...
    for(cluster_i in 1:c) {
      for(prot_i in 1:k) {
        if(is_inf(mu_bg[prot_i]*scale_use + mu_s_cp[cluster_i, prot_i]) || is_nan(mu_bg[prot_i]*scale_use + mu_s_cp[cluster_i, prot_i])){
        print("=======================");
        ...
        log_lik_clust_mat[pos1:pos2, cluster_i] = log_lik_clust_mat[pos1:pos2, cluster_i] + 
          log_zinb_vec2(x_indx[pos1:pos2, prot_i], unique_prot_x[indx_prot[prot_i,1]:indx_prot[prot_i,2]], 
                        mu_bg[prot_i]*scale_use + mu_s_cp[cluster_i, prot_i], phi_s_cp[cluster_i, prot_i], theta0_s_cp[cluster_i, prot_i]);
      }
    }
  }

  for(cell_i in 1:N) target += log_sum_exp(log_theta + log_lik_clust_mat[cell_i]');
